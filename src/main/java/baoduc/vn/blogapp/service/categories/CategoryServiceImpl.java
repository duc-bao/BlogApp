...
public class CategoryServiceImpl implements CategorySevice{
    ...
    public CategoriesDTO getCategory(int idCategory) {
        Category category = categoriesRepository.findById(idCategory).orElseThrow(() ->new ResourceNotFoundException("category","id", idCategory));
    ...
    public List<CategoriesDTO> getAllCategory() {
        ...
        return  categories.stream().map(category -> modelMapper.map(category, CategoriesDTO.class)).collect(Collectors.toList());
    }

    @Override
    public CategoriesDTO updateCat(CategoriesDTO categoriesDTO, int id) {
        Category category = categoriesRepository.findById(id).orElseThrow(()-> new ResourceNotFoundException("category", "id", id))   ;
